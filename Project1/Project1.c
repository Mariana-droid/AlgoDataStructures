/*
 * File:  projeto
 * Author:  Mariana Charneca, ist195635
...
void executa_N(){
    ...
    {
    scanf(" %[^:]",encomendas[id_encomenda].cliente);
    ...
    printf("Nova encomenda %d %s.\n",id_encomenda,encomendas[id_encomenda].cliente);
...
/*lista todos os produtos existentes no sistema por ordem crescente de preco. Se houver 2 ou mais produtos com o mesmo preco, devera imprimir esses por ordem crescente de `id` de produto
  * Formato de entrada: `l`
  * Formato de saida: Um bloco, e em que os produtos sao listados por ordem crescente de preco*/
...
/*lista o identificador da encomenda em que o produto dado ocorre mais vezes. Se houver 2 ou mais encomendas nessa situacao, devera imprimir a encomenda de menor `id`.
  * Formato de entrada: `m idp`
  * Formato de saida:
    * `Maximo produto <idp> <ide> <qtd>.` onde `<ide>` e o numero da encomenda em que `<idp>` ocorre mais vezes, sendo essa quantidade `<qtd>`
    * Nao devera listar nada caso o produto `<idp>` nao ocorra em nenhuma encomenda, ou caso nao haja encomendas*/
void executa_m(){
...
void executa_v(){
    ...
    {
        printf("%d %s.\n",ide,encomendas[ide].cliente);
    }
}
void quicksort_y(int ordenado[encomenda_max],int precos[encomenda_max],int first,int last){
    ...
    if(first<last){
      ...
      while(i<j){ /*comparar o preco do ordenado com o do pivot*/
         while(precos[ordenado[i]] <= precos[ordenado[pivot]] &&i<last){
            if (precos[ordenado[i]] == precos[ordenado[pivot]]){ /*se forem iguais comparar os ids*/ 
               if (encomendas[ordenado[i]].id > encomendas[ordenado[pivot]].id){
                    break;
                }
            }
            i++; }
         while(precos[ordenado[j]] >= encomendas[ordenado[pivot]] &&j>first){ /*comparar o preco do ordenado com o pivot*/
            if (precos[ordenado[j]] == encomendas[ordenado[pivot]]){ /*se forem iguais comparar os ids*/
                if (encomendas[ordenado[j]].id < encomendas[ordenado[pivot]].id){
                    break;
                }
            }
            j--;
      ...
      temp=ordenado[pivot];
      temp2 = precos[pivot];
      precos[pivot] = precos[j];
      ordenado[pivot] = ordenado[j];
      ordenado[j] = temp;
      precos[j] = temp2;
      /*fazer o quicksort a direita do pivot e a esquerda*/
      quicksort_y(ordenado,first,j-1);
      quicksort_y(ordenado,j+1,last);
...
void executa_Y(){
    int maior, i,e,total;
    int ordenado[encomenda_max];
    int precos[encomenda_max];
    maior = id_encomenda;
    total = 0;
    for (i=0; i < maior;i++){
        ordenado[i] = i; /*Ids dentro do ordenado igual ao produtos*/
        for (e=0; e< id_num; e++){
            if (encomendas[i].fila[e].stock != 0) /*se o stock for maior que zero*/
            {
                total = encomendas[i].fila[e].stock*encomendas[ide].fila[i].preco + total;/*somar ao preco total os precos vezes encomendas*/
            }
        }
        precos[i] = total;
        total =0;
    ...
    quicksort_y(ordenado,precos,0,maior-1);
    printf("Encomendas\n");
    for (i=0;i < maior;i++){ 
        printf("* %d %d\n",encomendas[ordenado[i]].id,precos[i]);
    }
